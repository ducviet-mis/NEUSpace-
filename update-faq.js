...
// Add imports
content = content.replace("import { BookOpen, CheckCircle, AlertCircle, ArrowRight, Plus, Star, Info, Sparkles } from 'lucide-react';", 
...
// Update TabType
content = content.replace("type TabType = 'shared' | 'required_extra' | 'electives';",
"type TabType = 'shared' | 'required_extra' | 'electives' | 'faq';");

// Add FAQ_DATA
const faqDataCode = `
const FAQ_DATA = [
  { q: "Phòng ban/ thầy cô nào phụ trách các vấn đề học cùng lúc 2 chương trình đào tạo?", a: "Thầy Sơn - Phòng Quản lý đào tạo NEU (Phòng 211- tòa A1)" },
  ...
  { q: "Làm thế nào để được chuyển điểm các học phần tương đương?", a: "Sinh viên đối chiếu giữa 2 chương trình đào tạo đang theo học để biết môn có thể đổi điểm. Thời gian đăng ký: từ ngày 1 đến ngày 8 hàng tháng.\\nMỗi lần điền form tương ứng với 1 môn.\\nhttps://forms.office.com/r/kTDujpnQKq" },
...
// Add openFaqIndex state
content = content.replace("const [selectedElectives, setSelectedElectives] = useState<Set<string>>(new Set());",
"const [selectedElectives, setSelectedElectives] = useState<Set<string>>(new Set());\n  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(null);");

// Find the electives button to insert faq button right after it
const electivesButtonRegex = /<button[\s\S]*?onClick=\{\(\) => setActiveTab\('electives'\)\}[\s\S]*?<\/button>/;
const match = content.match(electivesButtonRegex);
if (match) {
  const faqButton = `
              <button
  ...
  `;
  content = content.replace(match[0], match[0] + faqButton);
}
