...
const PROMPTS: Record<FeedbackTopic, { label: string; placeholder: string }> = {
...
export default function FeedbackPage() {
  ...
  const prompt = topics.length === 1
    ? PROMPTS[topics[0]]
    : {
        label: 'Bạn muốn chia sẻ điều gì với neuOS?',
        placeholder: 'Bạn có thể vừa báo lỗi, vừa đề xuất cách cải thiện hoặc một tính năng mới. Hãy mô tả theo từng ý để đội ngũ dễ xử lý...',
      };

  const toggleTopic = (topic: FeedbackTopic) => {
  ...
  const submitFeedback = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (topics.length === 0) {
      setSubmitError('Vui lòng chọn ít nhất một chủ đề góp ý.');
      return;
    }
    if (trimmedMessage.length < 12) {
      setSubmitError('Hãy chia sẻ ít nhất 12 ký tự để đội ngũ hiểu rõ góp ý của bạn.');
      return;
    }
        ...
        <div className="grid lg:grid-cols-[12rem_minmax(0,1fr)]">
          ...
          <div className="min-h-[27rem] px-5 py-7 sm:px-8 sm:py-9">
            {submitted ? <div className="flex min-h-[22rem] flex-col items-center justify-center text-center"><div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-300"><CheckCircle2 size={34} aria-hidden="true" /></div><h3 className="mt-5 text-2xl font-bold text-foreground">Cảm ơn bạn!</h3><p className="mt-2 max-w-md text-sm leading-relaxed text-foreground/70">Góp ý của bạn đã được gửi riêng tư đến đội ngũ neuOS. Cảm ơn bạn đã giúp ứng dụng tốt hơn mỗi ngày.</p><button type="button" onClick={restart} className="mt-6 inline-flex min-h-11 items-center justify-center rounded-xl border border-border px-4 text-sm font-semibold text-foreground transition-colors hover:bg-foreground/8">Gửi một góp ý khác</button></div> : step === 1 ? <div className="mx-auto max-w-xl">
              ...
              <p className="text-sm font-semibold uppercase tracking-[0.1em] text-cyan-700 dark:text-cyan-300">Bước 2 / 3</p><h3 className="mt-2 text-2xl font-bold text-foreground">Bạn muốn góp ý về điều gì?</h3><p className="mt-2 text-sm leading-relaxed text-foreground/70">Bạn có thể chọn một hoặc nhiều nhóm — ví dụ vừa báo lỗi, vừa đề xuất cách cải thiện.</p>
              <div className="mt-4 min-h-6 text-sm font-semibold text-cyan-700 dark:text-cyan-300" aria-live="polite">{topics.length ? `Đã chọn ${topics.length} chủ đề` : 'Chưa chọn chủ đề nào'}</div>
